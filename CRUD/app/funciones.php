...
function volcarDatostxt($tvalores){
    $fichero = @fopen(FILEUSER, "w") or die("No se puede abrir el fichero.");
    foreach ($tvalores as $valores) {
...
function cargarDatosjson (){
    ...
    $tabla = [];
    $fichero = file_get_contents(FILEUSER);
    $datos = json_decode($fichero, true);
...
function volcarDatosjson($tvalores){
    ...
    $fichero = json_encode($tvalores);
    file_put_contents(FILEUSER, $fichero);
...
function limpiarArrayEntrada(array &$entrada){
    ...
    foreach ($entrada as $id => $valor) {
        $nuevo[$id] = trim(htmlspecialchars($valor));
    }
